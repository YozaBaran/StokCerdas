-- ============================================================
-- DATABASE SCHEMA & SEED DATA: StokCerdas Platform
-- Designed for phpMyAdmin / MySQL / MariaDB
-- Backend: Node.js Express + mysql2 Pool
-- ============================================================

CREATE DATABASE IF NOT EXISTS `stokcerdas_db` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `stokcerdas_db`;
...
CREATE TABLE IF NOT EXISTS `products` (
  ...
  `safety_stock` DECIMAL(10,2) NOT NULL DEFAULT 3,
  `lead_time_days` INT NOT NULL DEFAULT 2,
  `supplier_id` VARCHAR(50),
  `expiry_tracking` TINYINT(1) DEFAULT 1,
...
-- 5. STOCK TRANSACTIONS TABLE (IMMUTABLE AUDIT TRAIL)
CREATE TABLE IF NOT EXISTS `stock_transactions` (
  `id` VARCHAR(50) NOT NULL PRIMARY KEY,
  `timestamp` DATETIME NOT NULL,
  `product_id` VARCHAR(50) NOT NULL,
  `product_name` VARCHAR(100) NOT NULL,
  `type` ENUM('Stock In', 'Stock Out', 'Stock Adjustment') NOT NULL,
...
-- 8. APP STATE BACKUP DUMP TABLE (FOR HYBRID SINGLE-JSON SYNC WITH PHPMYADMIN)
